...
CREATE OR REPLACE FUNCTION public.handle_new_user()
 ...
 SECURITY DEFINER
AS $function$begin
  -- Check if username exists before even trying the insert
  if exists (select 1 from public.profiles where username = new.raw_user_meta_data ->> 'username') then
    -- This error message will be sent back to your SvelteKit frontend
  ...
  values (
    ...
    2    -- Default hit tolerance/mercy metric
  );
